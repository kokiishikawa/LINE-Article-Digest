...
def is_over_limit(user_id: str) -> bool:
    """当日の利用回数が上限に達しているか確認する"""
    today = get_today()
    resp = table.get_item(Key={"userId": user_id, "date": today})
    item = resp.get("Item")
    if not item:
        return False
    return int(item.get("request_count", 0)) >= DAILY_LIMIT_COUNT


def record_usage(user_id: str) -> None:
    """当日の利用回数を DynamoDB に加算する"""
    today = get_today()
    table.update_item(
        Key={"userId": user_id, "date": today},
        UpdateExpression="ADD request_count :n",
